    ...
    <div class="row" >
        	...
        	<div class="col-sm-12 ">
        		<div class="z_bar-gray z_border_t z_border_b clearfix">
        		 	<form action="{{ route('business.rj_machine_monitor') }}" method="get">
		        			...
		        			<div class="clearfix z_mb10">
			        			...
			        			<div class="z_inputbox fl z_mr30">
			        				...
			        				<select name="is_open" class="z_w172 z_h24 input-sm form-control input-s-sm inline">
			        					...
			        					<option value="0" @if($is_open==='0') selected @endif>离线</option>
                            ...
                            <tbody>
                              ...
                              @foreach($ads as $k=>$dev)
	                               ...
	                               <td>{{$dev->is_disable=='1'?'禁用':'正常'}}</td>
									<td>
                                        <?php
                                        $status = DB::table(config('tables.base').'.rj_iot_machine_log')->where('machine_id',$dev->m_id)->orderBy('id','desc')->value('status');
                                        if($status=='1'){
                                            echo '异常';
                                        }else{
                                            echo '正常';
                                   ...
                                   <td>
                                   		<a href="{{ route('business.rj_machine_monitor_info',['id'=>$dev->m_id]) }}" class="btn btn-white btn-sm"><i class="fa fa-pencil"></i>  详情</a>                                         
                                    </td>
                                </tr>
                              @endforeach
                            @endif
