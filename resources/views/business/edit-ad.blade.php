...
<div class="wrapper wrapper-content animated fadeInUp">
    <div class="row">
        <form action="{{ route('business.edit-ad') }}" method="post" class="form-add-ad">
            {{ csrf_field() }}
                    ...
                    <div class="ibox-content">
                        <div class="row">
                            <div class="col-sm-8">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="tabs-container">
                                        <ul class="nav nav-tabs">
                                            <li class="switch-type @if(!empty($ad->content)) active @endif" data-type-index="1"><a
                                                        data-toggle="tab" href="#tab-1" aria-expanded="true"> 图文</a>
                                            </li>
                                            <li class="switch-type @if(!empty($ad->url)) active @endif" data-type-index="1"><a
                                                        data-toggle="tab" href="#tab-2" aria-expanded="false"> 超链接</a>
                                            </li>
                                            <li class="switch-type @if($ad->type == 3) active @endif" data-type-index="3"><a
                                                        data-toggle="tab" href="#tab-3" aria-expanded="false"> 套餐</a>
                                            </li>
                                            <input type="hidden" name="type" value="{{ $ad->type }}">
                                        </ul>
                                        <div class="tab-content">
                                            <div id="tab-1" class="tab-pane @if(!empty($ad->content)) active @endif">
                                                <div class="panel-body">
                                                    <script id="content" name="content" type="text/plain">{!! $ad->content !!}</script>
                                                </div>
                                            </div>
                                            <div id="tab-2" class="tab-pane @if(!empty($ad->url)) active @endif">
                                                <div class="panel-body">
                                                    <div class="form-group">
                                                        <label>链接</label>
                                            ...
                                            <div id="tab-3" class="tab-pane @if($ad->type == 3) active @endif">
                                                <div class="panel-body">
                                                    <div class="form-group">
                                                        <label>门店</label>
                                                            ...
                                                            @if(!empty($stores))
                                                                ...
                                                                @foreach($stores as $store)
                                                                    <option value="{{ $store->id }}"
                                                                            @if($store->id == $ad->store_id) selected @endif >{{ $store->name }}</option>
                                                                @endforeach
                                                            @endif
                                                        </select>
                                                    </div>
                                ...
                                <div class="form-group @if($ad->type == 3) hide @endif " id="stores">
                                    <label>投放门店</label>
                                    <div>
                                        @if(!empty($stores))
                                            @foreach($stores as $store)
                                                <label class="checkbox-inline i-checks">
                                                    <input type="checkbox"
                                                           @if(in_array($store->id,$relatedStores)) checked
                                                           @endif name="store_ids[]"
                                                           value="{{ $store->id }}">{{ $store->name }}</label>
                                            @endforeach
                                        @endif
                                    </div>
                                </div>
                                    ...
                                    <div>
                                        <div class="switch">
                                            <div class="onoffswitch">
                                                <input type="checkbox" @if($ad->flag == 1) checked
                                                       @endif class="onoffswitch-checkbox" id="rec1" name="flag"
                                                       value="1">
                            ...
                            <div class="col-sm-4">
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <div class="ibox-content no-padding border-left-right image-preview"
                                             style="margin-top:10px">
                                            <img class="img-preview-sm"
                                                 src="{{ config('static.base_url').'/'.$ad->path }}">
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <button class="btn btn-sm btn-primary btn-add-ad" type="button">保存</button>
                    </div>
                </div>
            </div>
        </form>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        imageUploader.on('uploadSuccess', function (file, response) {
            $('.image-preview').empty().append($('<img class="img-preview-sm">').attr('src', response.data[0].absolute_path).css({
                width: 160,
                height: 90
            }));
        ...
        function listPackages(storeId) {
            youyibao.getPackages(storeId, function (data) {
                var res = null;
                if(data.length > 0){
                    $.each(data, function (index, value) {
                        var select = '';
                        @if(!empty($ad->package_id))
                        if (value.id == "{{ $ad->package_id }}") {
                            select = 'selected';
                        } else {
                            select = '';
                        }
        ...
        var storeId = "{{ $ad->store_id }}";
        if (storeId > 0) {
            listPackages({{ $ad->store_id }});
        }

        $('.stores').change(function () {
            if (parseInt($(this).val()) > 0) {
                listPackages($(this).val());
            }
        });
