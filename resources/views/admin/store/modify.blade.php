...
@section('main')
            ...
            <div class="col-sm-12">
                <div class="ibox">
                    ...
                    <div class="ibox-content">
                        <div class="row">
                            <form action="{{ route('admin.business.store.modify') }}" class="forms">
                                {{ csrf_field() }}
                                <div class="col-sm-5">
                                    ...
                                    <div class="form-group">
                                        <label>电话</label>
                                        <input type="text" name="tel" placeholder="" class="form-control" value="{{$info->tel}}">
                                    </div>
                                    <div class="form-group">
                                        <label>城市</label>
                                        <select class="form-control m-b" name="city_id" id="city" cityid="{{$info->city_id}}">
                                            <option>正在加载...</option>
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        <label>门店地址</label>
                                        <input type="text" id="address" name="address" placeholder="" class="form-control" value="{{$info->address}}">
                                    ...
                                    <div class="form-group">
                                        <label>门店维度</label>
                                        <input type="text" name="latitude" id="latitude" placeholder="" class="form-control" value="{{$info->latitude}}">
                                    </div>
                                    <div class="form-group">
                                        <label>门店经度</label>
                                        <input type="text" name="longitude" id="longitude" placeholder="" class="form-control" value="{{$info->longitude}}">
                                    </div>
                                    <div class="form-group">
                                        <label>品牌</label>
                                                ...
                                                <div class="file-box">
                                                    <div class="file">
                                                        ...
                                                        <div class="image">
                                                            ...
                                                            <img alt="image" class="img-preview-sm" src="{{$img->path}}">
                                                        </div>
                                                        <div class="file-name text-center">
                                                            <button class="btn btn-warning btn-circle btn-del-photo" type="button">
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            $.get('/business/teacher/list/json',function(data){
                var html = '';
                var teacher = '{{$info->teachers}}';
                for(var i in data){
                    if(teacher == ''){
                        html += '<option value="'+data[i].id+'">'+data[i].name+'('+data[i].job+')</option>';
                        continue;
                    }
                    if(in_array(teacher.split(',') , data[i].id)){
                        html += '<option value="'+data[i].id+'" selected>'+data[i].name+'('+data[i].job+')</option>';
                    }else{
                        html += '<option value="'+data[i].id+'">'+data[i].name+'('+data[i].job+')</option>';
                    }
                }
            ...
            $.get('/city/list/json',function(data){
                var html = '';
                var cur_city_id = $('#city').attr('cityid');
                for(var i in data){
                    if(data[i].id == cur_city_id){
                        html += '<option value="'+data[i].id+'" selected>'+data[i].name+'</option>';
                    } else {
                        html += '<option value="'+data[i].id+'">'+data[i].name+'</option>';
                    }
    ...
    <script type="text/javascript">
        ...
        var marker = new AMap.Marker({
                position:["{{$info->longitude}}", "{{$info->latitude}}"]//位置
            })
