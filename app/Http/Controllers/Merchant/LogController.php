...
class LogController extends Controller{
	...
	public function show(Request $request){
        ...
        /* 获取帐号下可以查看的操作日志的用户名 */
        if(session('pid') == 0){
       		$allow_users = DB::table('bus_users')->select(['id','name'])->whereIn('id',$child_ids)->get();
        }

        /* 筛选用户 */
        $filter_user_id = $request->get('uid');
        if($request->has('uid') && intval($filter_user_id) && $filter_user_id > 0 ){
	...
	public function detail(Request $request){
		...
		if(in_array($log->action, [1,2,3])){
			...
			return view('business.log-detail-1',[
				'info' => $log,
				'action'=>$this->action
			]);
