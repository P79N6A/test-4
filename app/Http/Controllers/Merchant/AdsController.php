...
class AdsController extends Controller{
    ...
    public function add(Request $request){
        if($request->isMethod('post')){
            ...
            if($ads->save()){
                return $this->response(200,'添加成功！',route('business.ads.list'));
            }else{
                return $this->response(422,'处理失败，请重试！');
            }
        }

        return view('Merchant.ads.add');
    }
    ...
    public function modify(Request $request){
        ...
        if(empty($ads)) return redirect(route('business.ads.list'));
        ...
        if($request->isMethod('POST')){
            ...
            if($ads->save()){
                return $this->response(200,'修改成功！',route('business.ads.list'));
            }else{
                return $this->response(422,'处理失败，请重试！');
            }
        }

        return view('Merchant.ads.modify',[
            'info' => $ads
        ]);
    }
    ...
    public function delete(Request $request){
        ...
        if(AdsModel::destroy($id)){
            return $this->response(200,'删除成功！',route('business  .ads.list'));
        }else{
