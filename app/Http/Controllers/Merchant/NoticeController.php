...
class NoticeController extends Controller{
    ...
    public function lists(Request $request){
        ...
        return view('Merchant.notice.list',[
            'list' => $list
        ]);
    }
    ...
    public function add(Request $request){
        if($request->isMethod('post')){
            ...
            if($notice->save()){
                return $this->response(200,'添加成功！',route('business.notice.list'));
            }else{
                return $this->response(422,'处理失败，请重试！');
            }
        }

        return view('Merchant.notice.add');
    }
    ...
    public function modify(Request $request){
        ...
        if(empty($notice)) return redirect(route('business.notice.list'));
        ...
        if($request->isMethod('POST')){
            ...
            if($notice->save()){
                return $this->response(200,'修改成功！',route('business.notice.list'));
            }else{
                return $this->response(422,'处理失败，请重试！');
            }
        }

        return view('Merchant.notice.modify',[
            'info' => $notice
        ]);
    }
    ...
    public function delete(Request $request){
        ...
        if(NoticeModel::where('id',$id)->update(['disabled'=>1])){
            return $this->response(200,'删除成功！',route('business.notice.list'));
        }else{
