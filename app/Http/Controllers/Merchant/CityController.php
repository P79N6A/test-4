...
class CityController extends Controller{
    ...
    public function lists(Request $request){
        ...
        return view('Merchant.city.list',[
            'list' => $list
        ]);
    }
    ...
    public function add(Request $request){
        if($request->isMethod('post')){
            ...
            if($city->save()){
                return $this->response(200,'添加成功！',route('business.city.list'));
            }else{
                return $this->response(422,'处理失败，请重试！');
            }
        }

        return view('Merchant.city.add');
    }
    ...
    public function modify(Request $request){
        ...
        if($request->isMethod('POST')){
            ...
            if($city->save()){
                return $this->response(200,'修改成功！',route('business.city.list'));
            }else{
                return $this->response(422,'处理失败，请重试！');
            }
        }

        return view('Merchant.city.modify',[
            'info' => $city
        ]);
    }
